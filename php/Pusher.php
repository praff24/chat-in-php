...
class Pusher
{
    ...
    private $auth_key = '16118eb1440f8b51d212';
    private $secret = '1d84aa860991716a7f68';
    private $app_id = '362983';
    private $options = '';
     ...
     * @param string $secret
     * @param int    $app_id
     * @param bool   $this->options  [optional]
    ...
    public function __construct()
    {
        ...
        if (isset($this->options['encrypted']) &&
                $this->options['encrypted'] === true &&
                !isset($this->options['scheme']) &&
                !isset($this->options['port'])) {
            $this->options['scheme'] = 'https';
            $this->options['port'] = 443;
        }

        $this->settings['auth_key'] = $this->auth_key;
        $this->settings['secret'] = $this->secret;
        $this->settings['app_id'] = $this->app_id;
        ...
        if (isset($this->options['notification_host'])) {
            $this->settings['notification_host'] = $this->options['notification_host'];
        } else {
            $this->settings['notification_host'] = 'nativepush-cluster1.pusher.com';
        }

        // Set scheme for native notifications
        if (isset($this->options['notification_scheme'])) {
            $this->settings['notification_scheme'] = $this->options['notification_scheme'];
        } else {
        ...
        if (!array_key_exists('host', $this->settings)) {
            if (array_key_exists('host', $this->options)) {
                $this->settings['host'] = $this->options['host'];
            } elseif (array_key_exists('cluster', $this->options)) {
                $this->settings['host'] = 'api-'.$this->options['cluster'].'.pusher.com';
            } else {
